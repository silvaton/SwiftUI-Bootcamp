...
//
//  Created by Ton Silva on 23/2/24.
//
...
struct SignInWithAppleResult {
    let token: String
    let nonce: String
    let userFullName: PersonNameComponents
    let name: String?
    let email: String?
}
...
extension SignInAppleHelper: ASAuthorizationControllerDelegate {
    ...
    func authorizationController(controller: ASAuthorizationController, didCompleteWithAuthorization authorization: ASAuthorization) {
        ...
        let tokens = SignInWithAppleResult(token: idTokenString, nonce: nonce, userFullName: userFullName, name: name, email: email)
